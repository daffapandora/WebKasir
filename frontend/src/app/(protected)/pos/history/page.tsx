...
import { ArrowLeft, Search, Receipt, Eye, Printer } from 'lucide-react';
...
export default function POSHistoryPage() {
  ...
  const myTransactions = MOCK_TRANSACTIONS.filter(t => {
    if (search) {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-bg-primary)' }}>
      ...
      <header className="flex items-center gap-3 px-4 py-3 border-b" style={{ background: 'var(--color-bg-surface)', borderColor: 'var(--color-border-light)' }}>
        ...
        <div><h1 className="font-bold" style={{ color: 'var(--color-text-primary)' }}>Riwayat Transaksi</h1><p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{user?.name} — {myTransactions.length} transaksi</p></div>
      </header>
          ...
          {myTransactions.map(txn => (
            <div key={txn.id} className="card p-4 animate-fade-in">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{txn.customer_name || 'Pelanggan Umum'} • {txn.items.length} item</p>
                  <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{formatDateTime(txn.created_at)} • {getPaymentLabel(txn.payments[0]?.method)}</p>
                </div>
